...
\noindent The \textsc{Analyser} definition of Section \ref{sec:analyser} states it is a\textit{ set of methods for data processing and analysing, which allow to refute or confirm hypothesis and improve existing models trough empirical findings}. Figure \ref{fig:analyser-block-schema-impl}, which extends the original one presented in Chapter \ref{chap:heaven} in Figure \ref{fig:analyser-block-schema}, contains the block diagram for describing how the data processing methods are organised. 

In this Section we describe the block in the schema are implemented, which analysis they allows and which tools sustain each level of analysis. Notice that the relation between the hypothesis and the tools that sustain the analysis is deep, thus it is hard to generalise the investigation. Hypothesis depends on the the research question, while the tools are related to the nature of the data, which again concern the experiment.  However, there are some general meaningful characteristics, which are independent from both the hypothesis and experiment. 

The \textsc{Analyser} Design includes Five Analysis Levels (see Section \ref{sec:analyser}. Each of this level applies the Systematic Comparative Approach presented in Chapter \ref{chap:problem-settings}), build comparisons upon common and well-defined properties. 

Before starting with level description, Figure \ref{fig:analyser-block-schema-impl} shows the \textsc{Analyser} receives two input:
\begin{itemize}
\item the raw data form the experiments
\item the variables to build the analysis
\end{itemize}

In the original block schema (See Figure \ref{fig:analyser-block-schema}) both inputs directly enter the Steady State Identification Block (SSIB) and the Analysis Block (AB). However is worth to note that even if the Test Stand is designed to be system independent, it is a dynamic system as the ones we want to study. Thus, strange behaviours may happen while an experiment is running. In order to reduce and possible eliminate the outliers, multiple runs of the same experiment must be mediated obtaining the average measures.Indeed, both the Steady State Identification Block and Analysis Block require an automatic procedure, named pre-processing in Figure \ref{fig:analyser-block-schema-impl}, which averages the data of multiple executions of the same experiment and calculate the statistically relevant data. Time series describe how a dynamic system evolves over time, so it is meaningful to attempt hypothesis verification trough statistical values, which always consider the the Steady State to allow the generalisation of the insights. The most common metrics are average and standard deviation and maximum or minimum for a certain variable.

The pre-processing block ensure data reliability extrapolating a unique dataset from multiple executions. Once we have reliable data, the Steady State Identification Block can process them and evidence, w.r.t the variables received as input. This process determines if a solution has reached the Steady State condition for a certain variable. Automatic procedures to identify the State State condition exist, but they require dedicated studies which will be faced as future works. Currently, the SSIB is it no automated. It exploits data visualisation techniques, to identify, if and when Steady State condition is reached is necessary and then it excludes the initial warm-up phase form the data. Anyhow, we know that the graphical method is limited, it must be applied for each system variable, because they may reach the equilibrium at different times.

Analysis Block hides the level where the comparative research approach is declined either to the visual analysis or statistical investigation. The graphical analysis method is more qualitative then the second one, but reading the information presented in graphical way can be preferred in those case where numerical data are not clear. On the other hand, the statistical investigation method demands more complex instruments to interpret the data, but allows to answer also elaborate questions with simpler answers. Following we present for all the Analysis level the method involved in the current implementation.
\pagebreak

\textsc{Level 0 - Dashboards}

\noindent Figure \ref{fig:dashboard-example} contains an example of the possible Dashboard representations. The aim of dashboard is representing the multiple solutions in a n-variable space. This kind of representation allow to define solution dominance, w.r.t the involved variable. It is easy to state with such a visual tool which solution, if any, is better then another one within an experiment or which experimental setting dominates the other fixing the studied RSP Engine.
...
Tables \ref{tab:comp-tables} (a) and (b) show two examples of statistical investigation. Table \ref{tab:comp-tables}.a contains the symbolic comparison of two solution over a given variable, while  Table \ref{tab:comp-tables}.b offers a deeper details level, showing how much a solution is better than the other. How to choose the proper level depends on the needs of the research.

What is meaningful for Level 1 is also how the results are disposed within the tables. Tables axes represent the variation of two different experiment properties A and B.  Different experiments influence the behaviour of an RSP Engine in different ways. We can move on the horizontal axis of Table \ref{tab:comp-tables}.a, variation the Condition A to appreciate those differences. 
...
\noindent Level 2 exploits the same experiment layout of Level 1, but it fills the tables cell with different charts for each experiment. Figure \ref{fig:pattern-examples} (a) and (b) are two examples. They report two possible memory analysis: in time domain (a) and frequency domain (b). The aim of this level is to identify patterns within the RSP Engine behaviour for a certain variable. For is reason it is necessary to appreciate together multiple experiments results. 
...
\noindent Finally, the lowest level of analysis focuses on single graphical visualisation. Figure \ref{fig:visual-comp} contains two possible analysis. Plotting multiple variables over the same chart allow to better understand their relation. For example we can state if one reaches the Steady State before the other as happens for Latency w.r.t Memory in Figure \ref{fig:visual-comp}.b. Comparing different experiment over the same variable allow similar observation too, focusing on the relation between the experimental settings. In summary:

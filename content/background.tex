...
Finally, the third and last class of operator is the Relation-to-Stream (R2S) ones. The transformed relation has to be set as output trough this an additional operator.
...
\subsection{Continuous Extension of SPARQL}l\label{sec:continuous-sparql}

\subsection{RDF Stream Processing Engine}\label{sec:rspengine}

Sections above report that RDF Stream Processing has three basic building blocks: 1) RDF streams, we introduced in Section \ref{sec:rdfstream} 2) continuos extensions of SPARQL, presented in \ref{sec:continuous-sparql} and 3) reasoning algorithms able to cope with rapidly changing information. 

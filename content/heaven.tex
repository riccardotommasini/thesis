...
Finally, baselines \textsc{Simplicity} comes from the registered query $\mathcal{Q},$ and the entailment regime in use. $\mathcal{Q}$ should be eligible in terms of reasoning, which means having an high materialisation effort of the implicit information entailed by the content of the window, given the ontology chosen by the user. The entailment regime should be a an RDF(S) with a good trade off between complexity and the normative semantics and the core functionalities. %Moreover, \textsc{Simplicity} is also provided by the external time control, which simplifies the system workflow.
...
We follow the researcher vision (ii) and thus in this section define the methods that compose the analysis. Further details of the implemented tools implement that sustain our investigation can be found in Section \ref{sec:analyser-impl}.

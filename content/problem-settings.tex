...
\begin{enumerate}
\item[R.13] a Solution: it solves the problem entirely, without concerning about performance. % devono risolvere interamente il problema, nel nostro caso devono essere complete e sound rispetto ad entailment regime che si è scelto
\item[R.14] Elementary: it requires the minimum effort in terms of design. A baseline does not represent a innovative solution w.r.t. the state of the art.  % non devono essere soluzioni smart, devono chiedere il minimo effort
\item[R.15] Valid: the performance measurements must be comparable with smarter solution and their comparison must have sense in a realistic context. %e' inutile se sono sopra un'ordine di grandezza ad esempio in latenza, rispetto ad una soluzione commerciale. probabilmente è sbagliata la domanda di ricerca
\item[R.16] Relevant: it must cover one of the most relevant variations in theoretical solution, in agreement with the case-oriented nature of CSRA. %la soluzione che coprono deve essere una delle immediate soluzioni del problema iniziale: Graph vs Statement o Inc vs Naive, tempo controllato esternamente, ecc
\item[R.17] Simple: the design of the solution it implements must be easy to understand. In this way is possible to formulate hypothesis from a theoretical viewpoint. Obviously there are no guarantees about the experimental verification.  %non devono offrire api più complesse rispetto a un RSPEngine tradizionale.
\end{enumerate}

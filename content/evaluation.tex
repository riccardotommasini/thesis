...
In Section \ref{sec:eval-level3} we have included an example of Level 3 Intra-Experiment comparison. Here we present the alternative method developed for Level 3: Inter Experiment comparison. We explained the design of Step Response in Section \ref{sec:step-es}, stating that the aim of this experiment set is to show \name potential. 

Figures \ref{fig:level3-step} (a) and (b) show an example of Inter-Experiment comparison. In the figure we draw the Latency and the Memory series for three experiment:
\begin{itemize}
...
All these tests have a Number of Slot in the window of 10. In general, DSMS does not allow to change the window dimension during the execution, thus experiment like Step Response ones, must be design to control other variables, in this case the \textit{CTEvent} size.

...
SOAK testing shows the system dynamics, injecting into the RSP Engine in use a constant and continuous input flow (see Section~\ref{sec:software-testing}). We maintain constant the number of contemporary triples in the RDF Stream through the \textit{RDF2RDFStream} module. %Moreover, Section~\ref{sec:query} describe how we variates the query between experiments.
...
Experiment number 5 ( the \text{CTEvent} Size = 10000 a slot Nnumber = 1=) did not terminate correctly for all the baselines, due to an execution error and thus, its results are not part of our evaluation.

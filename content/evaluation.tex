...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.50\linewidth]{images/dashboard-legend}	
...
SOAK Testing is a standard testing procedure for dynamic systems (see Section \ref{sec:software-testing}). However, the goal of this chapter is to show how \name works and thus showing its potential, providing more than one use case of the comparative RSP Engine research.
...
In this section we tried a different approach: we compare the results of Step Response experiments with the SOAK ones that represent the starting and ending condition. 
...
Notice that we can identify, trough the Steady State Identification Block, two possible Steady State conditions: before and after the step.  Thus, Figure \ref{fig:level-0-step-dashboard-1} contains the dashboard representation of the two phases of the Step Response Test. Moreover in Figure \ref{fig:level-0-step-dashboard-2} we include the results of the relative SOAK experiments. 
